...
#         base_group_save_path = os.path.join(self.cfg.save_path, "superpoint_result")
#         make_dirs(save_path)
...
#             if os.path.isfile(pred_save_path) and os.path.isfile(group_save_path):
#                 logger.info(
#                     "{}/{}: {}, loaded pred and group index.".format(
#                         idx + 1, len(self.test_loader), data_name
#                     )
#                 )
#                 pred = np.load(pred_save_path)
#                 # group_indices = np.load(group_save_path, allow_pickle=True).item()
#                 # raw_to_super_index = group_indices["raw_to_super_index"]
#                 # object_raw_to_super_index = group_indices["object_raw_to_super_index"]
#
...
#                 # 初始化全局索引张量
#                 num_points = segment.size  # 场景中原始点的总数
#                 raw_to_super_index_global = torch.empty(num_points, dtype=torch.int64).fill_(-1).cuda()
#                 object_raw_to_super_index_global = torch.empty(num_points, dtype=torch.int64).fill_(-1).cuda()
...
#                     idx_part = input_dict["index"]  # 当前 fragment 的全局索引
#                     with torch.no_grad():
#                         output = self.model(input_dict)
#                         pred_part = output["seg_logits"]
#                         raw_to_super_index = output["pred_in_component"]
#                         object_raw_to_super_index = output["pred_in_component_object_level"]
#                         # print("raw_to_super_index_len:", raw_to_super_index.numel())
#                         # # 填充全局索引张量
#                         # raw_to_super_index_global[idx_part] = raw_to_super_index.cpu()
#                         # object_raw_to_super_index_global[idx_part] = object_raw_to_super_index.cpu()
#
...
#                         #     bs = be
#                             # 使用 bs 和 offset 累加到全局索引张量中
#                         bs = 0
#                         for be in input_dict["offset"]:
#                             # 索引映射到全局
#                             raw_to_super_index_global[idx_part[bs:be]] = raw_to_super_index[bs:be]
#                             object_raw_to_super_index_global[idx_part[bs:be]] = object_raw_to_super_index[bs:be]
#                             pred[idx_part[bs:be], :] += pred_part[bs:be]  # pred 的累加逻辑
#                             bs = be
#
#                     # xyz = data_dict["points"]
#                     # 拼接所有批次的索引数据
#                     # raw_to_super_index = torch.cat(raw_to_super_index_list, dim=0)
#                     # object_raw_to_super_index = torch.cat(object_raw_to_super_index_list, dim=0)
#
#                     logger.info(
...
#                 # 保存原始点到不同层级超点的索引关系
#                 group_data = {
#                     "raw_to_super_index": raw_to_super_index_global.cpu().numpy(),
#                     "object_raw_to_super_index": object_raw_to_super_index_global.cpu().numpy(),
#                 }
...
#             else:
#                 pred = torch.zeros((segment.size, self.cfg.data.num_classes)).cuda()
#                 # 初始化全局索引列表，用于统计每个原始点被分配到的超点索引
#                 num_points = segment.size  # 场景中原始点的总数
#                 raw_to_super_index_global = [[] for _ in range(num_points)]
#                 object_raw_to_super_index_global = [[] for _ in range(num_points)]
#
#                 for i in range(len(fragment_list)):
#                     fragment_batch_size = 1
#                     s_i, e_i = i * fragment_batch_size, min(
...
#                         if self.cfg.empty_cache:
#                             torch.cuda.empty_cache()
#                         # 使用 bs 和 offset 累加到全局索引列表中
#                         bs = 0
#                         for be in input_dict["offset"]:
#                             for j in range(bs, be):
#                                 global_idx = idx_part[j].item()
#                                 raw_to_super_index_global[global_idx].append(raw_to_super_index[j].item())
#                                 object_raw_to_super_index_global[global_idx].append(object_raw_to_super_index[j].item())
#                             pred[idx_part[bs:be], :] += pred_part[bs:be]  # pred 的累加逻辑
#                             bs = be
#
#                     logger.info(
...
#
#                 # 统计每个原始点的最终超点索引（投票机制）
#                 raw_to_super_index_final = np.array([
#                     max(set(lst), key=lst.count) if lst else -1 for lst in raw_to_super_index_global
#                 ])
#                 object_raw_to_super_index_final = np.array([
#                     max(set(lst), key=lst.count) if lst else -1 for lst in object_raw_to_super_index_global
#                 ])
#
#                 np.save(pred_save_path, pred)
#                 # 保存原始点到不同层级超点的索引关系
#                 group_data = {
#                     "raw_to_super_index": raw_to_super_index_final,
#                     "object_raw_to_super_index": object_raw_to_super_index_final,
#                 }
#                 np.save(group_save_path, group_data)
#
#             if (
#                     self.cfg.data.test.type in ["ScanNetDataset", "ScanNet200Dataset"]
#             ):
#                 np.savetxt(
#                     os.path.join(save_path, "submit", "{}.txt".format(data_name)),
#                     self.test_loader.dataset.class2id[pred].reshape([-1, 1]),

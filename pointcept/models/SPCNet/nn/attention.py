...
from pointcept.models.SPCNet.utils.nn import build_qk_scale_func
...
class SelfAttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        if not isinstance(k_rpe, bool):
            ...
            self.k_rpe = nn.Linear(dim, qk_rpe_dim) if k_rpe else None

        if not isinstance(q_rpe, bool):
            self.q_rpe = q_rpe
        else:
            self.q_rpe = nn.Linear(dim, qk_rpe_dim) if \
    ...
    def forward(self, x, edge_index, edge_attr=None, device=None):
        """
        :param x: Tensor of shape (N, Cx)
            Node features
        :param edge_index: LongTensor of shape (2, E)
            Source and target indices for the edges of the attention graph.
        :param edge_attr: FloatTensor or shape (E, Ce)
            Edge attributes for relative pose encoding.
        :param device: Torch device to perform calculations on.
        :return:
        """
        ...
        if self.in_proj is not None:
            x = self.in_proj(x).to(device)

        # Compute queries, keys and values
        qkv = self.qkv(x).to(device)
        ...
        s = edge_index[0]  # [E]
        t = edge_index[1]  # [E]
        q = q[s].to(device)  # [E, H, D]
        k = k[t].to(device)  # [E, H, D]
        v = v[t].to(device)  # [E, H, C // H]

        # Apply scaling on the queries
        q = q * self.qk_scale(s).to(device)

        # Relative positional encoding (RPE) adjustments
        if self.k_rpe is not None and edge_attr is not None:
            rpe = self.k_rpe(edge_attr).to(device)
            if self.heads_share_rpe:
                rpe = rpe.repeat(1, H).to(device)
            k = k + rpe.view(E, H, -1)

        if self.q_rpe is not None and edge_attr is not None:
            if self.q_on_minus_rpe:
                rpe = self.q_rpe(-edge_attr).to(device)
            else:
                rpe = self.q_rpe(edge_attr).to(device)
            if self.heads_share_rpe:
                rpe = rpe.repeat(1, H).to(device)
        ...
        if self.k_delta_rpe is not None:
            rpe = self.k_delta_rpe((x[edge_index[1]] - x[edge_index[0]]).to(device))
            if self.heads_share_rpe:
                rpe = rpe.repeat(1, H).to(device)
            k = k + rpe.view(E, H, -1)
        ...
        if self.q_delta_rpe is not None:
            if self.q_on_minus_rpe:
                rpe = self.q_delta_rpe((x[edge_index[0]] - x[edge_index[1]]).to(device))
            else:
                rpe = self.q_delta_rpe((x[edge_index[1]] - x[edge_index[0]]).to(device))
            if self.heads_share_rpe:
                rpe = rpe.repeat(1, H).to(device)
            q = q + rpe.view(E, H, -1)
        elif self.k_delta_rpe is not None and self.qk_share_rpe and edge_attr is not None:
            if self.q_on_minus_rpe:
                rpe = self.k_delta_rpe((x[edge_index[0]] - x[edge_index[1]]).to(device))
            else:
                rpe = self.k_delta_rpe((x[edge_index[1]] - x[edge_index[0]]).to(device))
            if self.heads_share_rpe:
                rpe = rpe.repeat(1, H).to(device)
        ...
        if self.v_rpe is not None and edge_attr is not None:
            rpe = self.v_rpe(edge_attr).to(device)
            if self.heads_share_rpe:
                rpe = rpe.repeat(1, H).to(device)
            v = v + rpe.view(E, H, -1)

        # Compute compatibility scores from the query-key products
        compat = torch.einsum('ehd, ehd -> eh', q, k).to(device)  # [E, H]

        # Compute the attention scores with scaled softmax
        attn = softmax(compat, index=s, dim=0, num_nodes=N).to(device)  # [E, H]

        # Optional attention dropout
        if self.attn_drop is not None:
            attn = self.attn_drop(attn).to(device)

        # Apply the attention on the values
        x = (v * attn.unsqueeze(-1)).view(E, self.dim).to(device)  # [E, C]
        x = scatter_sum(x, s, dim=0, dim_size=N).to(device)  # [N, C]

        # Optional linear projection of features
        if self.out_proj is not None:
            x = self.out_proj(x).to(device)  # [N, out_dim]

        # Optional dropout on projection of features
        if self.out_drop is not None:
            x = self.out_drop(x).to(device)  # [N, C or out_dim]
